			...
			<?php foreach ($data_pesanan as $d) { ?>
				<tr>
					...
					<td>
						...
						<abbr title="Edit"> <a href="javascript:void(0)" onclick="edit('<?= $d['id_pesanan'] ?>')" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#Modal"><i class="fas fa-edit"></i></a></abbr>
...
<script type="text/javascript">
	function edit(id) {
		$('#Modal').modal('show');
		$.ajax({
			url: "<?= site_url('Admin/detailpesanan') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data) {
				var i;
				for (i = 0; i < data.length; i++) {
					...
					$('#lokasi_berangkat').val(data[i].lokasi_berangkat);
					$('#trayek').val(data[i].trayek);
					$('#tgl_mulai').val(data[i].tgl_mulai);
					$('#tgl_selesai').val(data[i].tgl_selesai);
					$('#jml_pax').val(data[i].jml_pax);
				}
			}
		});
	}
